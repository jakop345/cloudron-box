    ...
    database.beginTransaction(function (error, conn) {
        ...
        conn.query('INSERT INTO apps (id, appStoreId, version, installationState, location, accessRestriction) VALUES (?, ?, ?, ?, ?, ?)',
            ...
            async.eachSeries(Object.keys(portBindings), function iterator(containerPort, callback) {
                conn.query('INSERT INTO appPortBindings (hostPort, containerPort, appId) VALUES (?, ?, ?)',
...
function updateWithConstraints(id, app, constraints, callback) {
    ...
    database.beginTransaction(function (error, conn) {
        ...
        async.eachSeries(Object.keys(portBindings), function iterator(containerPort, callback) {
            ...
            conn.query('UPDATE appPortBindings SET hostPort = ? WHERE containerPort = ? AND appId = ?', values, callback);
