...
start_postgresql() {
    ...
    docker run -dtP --name=postgresql -v "${postgresqldatadir}:/var/lib/postgresql" -v /tmp/postgresql_vars.sh:/etc/postgresql/postgresql_vars.sh girish/postgresql:0.1.0 >/dev/null
}
...
start_mysql() {
    ...
    docker run -dP --name=mysql -v "${mysqldatadir}:/var/lib/mysql" -v /tmp/mysql_vars.sh:/etc/mysql/mysql_vars.sh girish/mysql:0.1.0 >/dev/null
}
